...
def q3_433(set):
    ...
    # print(c_l)
    # goal = pd.DataFrame() # 新建df新表
    # 阵型 4-3-3['GK', 'RB', 'CB', 'CB', 'LB', 'CDM', 'CM', 'CAM', 'RW', 'ST', 'LW']
    g = ['GK', 'RB', 'CB', 'CB', 'LB', 'CDM', 'CM', 'CAM', 'RW', 'ST', 'LW']
    for r in range(605000):
        ...
        # if glmax < colaver: # 取大
            ...
            gl.to_csv('goal433.csv',encoding='UTF-8',index=False) # 覆盖存储
    os.rename('goal433.csv', 'goal433_'+str(round(colsum,2))+'.csv') # 将分数加到文件名里
    # os.rename('goal433.csv', 'goal433_'+str(round(colaver,2))+'.csv') # 将分数加到文件名里
    # return colaver
    return colsum
set = "slist.csv"
total = q3_433(set)
